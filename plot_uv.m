function [ur vr]=plot_uv(url, timestamp, lyr, uname, vname, scl)
% quiver plot for vector fields in ROMS and COAWST
%
% [ur vr]=plot_uv(url, timestamp, lyr, uname, vname, scl)
% 
...
quiver(lon(2:end-1,2:end-1), lat(2:end-1,2:end-1), ur, vr, scl)
axis equal
title({sprintf('Vector plot for %s, %s',uname,vname); sprintf('Layer: %d, Date: %s', lyr, datestr(t(ti)))},...
    'interpreter', 'none')
